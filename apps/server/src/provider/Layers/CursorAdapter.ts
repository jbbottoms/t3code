...
export interface CursorAdapterLiveOptions {
  /** Driver kind stamped onto sessions, events, and errors. Defaults to `cursor`. */
  readonly providerKind?: ProviderDriverKind;
  /** ACP runtime factory. Alternate standard-ACP drivers can reuse this adapter. */
  readonly makeAcpRuntime?: typeof makeCursorAcpRuntime;
  /** Human-readable provider name used in diagnostics. Defaults to `Cursor`. */
  readonly providerLabel?: string;
  /** Register Cursor's non-standard ACP extension methods. Defaults to true. */
    ...
    const sendTurn: CursorAdapterShape["sendTurn"] = (input) =>
      Effect.gen(function* () {
        ...
        return yield* Effect.gen(function* () {
          ...
          // Let the ACP runtime register its active prompt before publishing
          // `turn.started`. Otherwise an immediate UI Stop can race ahead of
          // `session/prompt`; the agent sees a cancel for no active request and
          // then runs the prompt to completion.
